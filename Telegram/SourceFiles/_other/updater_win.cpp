...
QString UpdaterUrl() {
	return u"https://raw.githubusercontent.com/greenyarik0505-jpg/FluxGramDesktop/main/"_q;
}
		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"FluxGram.exe") && exeName != L"FluxGram.exe") {
	...
	for (size_t i = 0; i < from.size(); ++i) {
		...
		do {
			...
			if (!copyResult) {
				...
				WCHAR errMsg[2048];
				wsprintf(errMsg, L"Failed to update FluxGram :(\n%s is not accessible.", tofname.c_str());
					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								const WCHAR *appURL = L"https://desktop.FluxGram.org";
...
static const WCHAR *_programName = L"FluxGram"; // folder in APPDATA, if current path is unavailable for writing
